...
- **L6 `synthesize.py`** — deterministic backbone builds the iMessage digest (≤1800 chars, decision-first, with per-call entry/stop/target/conviction detail) + full markdown report; optional `--llm` relay (`prompts/desk_note.md`) writes an account-grounded `## Thesis` / `## What I Expect` (scenario+probability predictive read) / `## Game Plan`. Writes `reports/desk_<date>.md` (+ `_zh.md` translation when `DESK_LANG=zh`; code default is `en`). **Delivery (`deliver()`):** the English report is always archived; when `DELIVER_LANG=zh` it is `translate_to_zh()` → `supervise_zh()` first. With `ALERT_CHANNEL=email` it ships as **ONE Gmail** (digest on top, full report below) and **nothing is truncated** — `_email_channel()` lifts every phone-era cap (the digest's `IMSG_LIMIT`, `_short_reason`'s 96 chars, the calls-table `_cell` 140, the ideas/activity list caps); otherwise it's the legacy phone path (short digest + `send_report()` chunked 1800-char parts, caps enforced). `research.REASON_CHARS` (1200, `DESK_REASON_CHARS`) bounds how much analyst prose is stored on a call in the first place. Translation is CHUNKED via `translation_chunks()` (parallel OpenRouter chat calls via `OPENROUTER_API_KEY`, model from `BOT_QUICK_LLM`, default `deepseek/deepseek-v4-pro`): it groups small `## ` sections **and SPLITS oversized ones on line boundaries** — a long section (the Calls table with unclipped reasons) sent whole overruns `max_tokens`, returns truncated, drops figures, and silently falls back to English. `supervise_zh` uses the same chunker with `group=False` so it only re-touches sections that still hold English. A chunk is accepted only if it preserves every `$`/`%`/level (figures locked, `_tokens_preserved`) **and is no less Chinese than its source** (`_cjk_ratio(zh) >= _cjk_ratio(src)`) — an absolute "≥40% CJK" bar used to reject the figure-dense table rows, whose faithful translation is legitimately <40% CJK because tickers/prices dominate. A failed chunk keeps its English so delivery never breaks. **`_tokens_preserved` compares figures as NUMBERS, not glyphs:** it takes the `$`/`%` figures from the source and only requires each to reappear in the output as that number, because a faithful translation renders `$28.40` as `28.40美元` — demanding the literal `$` used to reject good translations and left the figure-dense sections (the Calls table, headlines) in English. The request sets `reasoning:{enabled:false}` — a reasoning model (the previous GLM-5.2 default) otherwise burns the whole `max_tokens` budget on chain-of-thought and returns `content=null`, silently failing every chunk back to English. `supervise_zh()` is a final QA re-read that only re-touches sections still containing English prose (the owner's "fully-Chinese guarantee"). This is separate from the read-only `--llm` claude enrichment relay.
...
- **`notify.py`** — the single "send a message" path. `ALERT_CHANNEL` picks the order: **`email`** → Gmail SMTP **only** (`ALERT_GMAIL_USER`/`ALERT_GMAIL_APP_PASSWORD` send to `ALERT_EMAIL`; deliberately no Mail.app/iMessage fallback so a long report is never truncated nor sent from the wrong account — it fails loudly instead) · `imessage` → iMessage only · `auto` (default) → iMessage → Mail.app → SMTP. A macOS notification is always best-effort. Triggered on analysis failure, broker rejection, ALERT.
